...
function main() {
    $phpbin httpstat.php $@ 2>&1
}

function main_silent() {
    $phpbin httpstat.php $@ >/dev/null 2>&1
}
...
out=$(HTTPSTAT_SHOW_BODY=true HTTPSTAT_SAVE_BODY=true main $https_url)
...
out=$(HTTPSTAT_SHOW_BODY=true HTTPSTAT_SAVE_BODY=false main $https_url)
echo "$out" | grep -q 'is truncated'
assert_exit 0